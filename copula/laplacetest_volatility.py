...
current_model3[0] = 100
current_model3[1] = 200
current_model3 = np.sort(current_model3)

current_model4 = np.zeros(numbrk, dtype=int)
current_model4[numbrk - 1] = 400

current_model5 = np.zeros(numbrk, dtype=int)
current_model5[numbrk - 1] = 70
current_model5 = np.sort(current_model5)
...
for i in range(0, itera):
    ...
    if i >= 10 and i % 10 == 0:
        ...
        # Updates zita #
        if i >= 500 and i % 500 == 0 and i < burnin:
            print(i)
        ...
        if np.random.uniform(low=np.nextafter(0.0, 1.0)) < 0.25:
            ...
            current_model5 = np.vstack((current_model5, result["currentModel"]))
